...
产出双文档——**调研报告**（结论层，面向阅读）+ **素材登记簿**（证据层，可追溯、可复查）。
...
2. 选定 `web_search` 与 `web_fetch` 各一个（可同一工具兼任），在首次回复中用一句话报告："本次调研将使用 <工具> 搜索、<工具> 抓取"。该报告与阶段 1/3 的选项**合并同一次回复**呈现，不单独占用一轮交互。
3. 无任何可用工具：如实说明"当前环境无联网搜索能力，无法调研"并终止。禁止用模型内部知识冒充调研结果。

## 阶段 1 · 预调研校准（本 skill 的核心）

仅当以下条件满足**其一**才执行（明确、无歧义的简单问题直接进阶段 3）：
...
- 含方向勾选在内最多 **3 轮**；仍未确认则选定最优假设（陈述理由）继续调研，并在报告元信息中**显式声明所做假设**，防止无限追问死循环。
...
- 给出推荐档位 + **一句话理由**，用户可改；用户不回应则直接按推荐档执行，并在报告中注明"采用推荐档"。

## 阶段 4 · 执行纪律

- **增量登记**：每轮搜索的收尾步骤就是写登记簿（新增条目或排除条目），先到先编号、禁止事后补写。
...
- **max 分阶段**：先交报告大纲（分节 + 来源策略）请用户确认，再分节执行、分节汇报；用户可随时纠偏或中断，中断时交付已完成部分与登记簿。

## 阶段 5 · 交付（双文档一起交付）

**调研报告**（摘要 → 主要发现 → 来源引用 → 局限与未覆盖）：

- 头部元信息：档位、日期、工具组合、假设声明（无假设时标注"无"）；敏感领域加"信息整理，非决策建议"。
- 关键结论内联 `[S#]` 编号（链接与访问日期见登记簿）；来源引用按 官方 / 权威媒体 / 社区观点 三层分组。
...
- **追问 / 深挖某一节**：以登记簿为知识底座作答并引用 `[S#]`，不重新搜索；登记簿不足以回答时明说"需补充检索"并征询，不静默重搜。
- **质疑复查**：用户质疑某结论时，只沿该结论在登记簿中的支撑条目复查证据链（重读核验 → 维持 / 修正 / 推翻），同步更新登记簿状态与报告对应结论，不全量重跑。
